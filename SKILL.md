...
| Railroading | X/5 | ... |
| Setup & Config | X/5 or N/A | ... |
| Scripts & Code | X/5 or N/A | ... |
| Memory/State | X/5 or N/A | ... |
| Context Efficiency | X/5 or N/A | ... |
| **Overall** | **X / Y 維度** | N/A 維度從分母排除；Context Efficiency 存在合理性 flag 另行標註 |

### 🔴 Critical Issues
[會讓 skill 完全失效或嚴重降低可用性的問題，例如整個必要章節缺失、結構性錯誤]
...
* 別把 checklist 念出來：review 的輸出是給人看的診斷報告，只提有問題的維度，沒問題的簡單帶過
* 先問用途再動手：全面審查和「為什麼不觸發」是完全不同的任務，沒問清楚就做完整 review 往往浪費雙方時間
